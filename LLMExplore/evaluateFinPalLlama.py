import csv
# Import the agent class and its dependencies from the modified finPal_agent.py
# 'llm_with_tools' is now the bound LLM chain
from finPalLLamaChat import FinPalAgent, llm_with_tools, tools, SYSTEM_MESSAGE_CONTENT 
...
# Pass llm_with_tools to the agent for evaluation
finPal_agent = FinPalAgent(llm_chain=llm_with_tools, tools=tools, system_message_content=SYSTEM_MESSAGE_CONTENT)
print("FinPalAgent instance created for evaluation.")

# --- 3. Define Output CSV File ---
output_csv_file = 'test_results_llama.csv'
